...
import org.radarcns.redcap.config.RedCapManager.getInfo
import org.radarcns.redcap.managementportal.Subject
import org.radarcns.redcap.util.IntegrationUtils
import org.radarcns.redcap.util.IntegrationUtils.LOGGER
...
import org.radarcns.redcap.util.IntegrationUtils.REDCAP_RECORD_ID_1
...
import org.radarcns.redcap.util.IntegrationUtils.updateProjectAttributes
...
class IntegratorTest {
    private val mpIntegrator = MpIntegrator(mpClient)
    ...
    private val testAttributes: MutableMap<String, String> = mutableMapOf(
        Pair(REDCAP_ATTRIBUTE_1, REDCAP_ATTRIBUTE_1_VAL),
        Pair(REDCAP_ATTRIBUTE_2, REDCAP_ATTRIBUTE_2_VAL)
    )

    private val attributeKeys: List<String> = listOf(
        REDCAP_ATTRIBUTE_1, REDCAP_ATTRIBUTE_2, IntegrationData.SUBJECT_ID_LABEL
    )

    private val redCapClient: RedCapClient = mock {
        on { fetchFormDataForId(attributeKeys, REDCAP_RECORD_ID_1) } doReturn testAttributes
    }
    private val redCapIntegrator: RedCapIntegator = RedCapIntegator(redCapClient)
    ...
    fun updateAttributesInMpTest() {
        ...
        assertEquals(subject.humanReadableIdentifier, HUMAN_READABLE_ID)
    }

    @Test
    @Throws(IOException::class, URISyntaxException::class)
    fun updateAttributesInMpWhenSubjectExistsTest() {
        testAttributes[REDCAP_ATTRIBUTE_1] = REDCAP_ATTRIBUTE_1_VAL_2
        ...
        assertEquals(subject.attributes, attributes)
        assertEquals(subject.attributes[REDCAP_ATTRIBUTE_1], REDCAP_ATTRIBUTE_1_VAL_2)
        assertEquals(subject.humanReadableIdentifier, HUMAN_READABLE_ID)
    }
